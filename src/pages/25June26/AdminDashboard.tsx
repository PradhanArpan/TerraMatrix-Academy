...
const defaultContentCategoryMap: Record<ContentCategoryKey, string[]> = {
  courses: courseCategories,
  videos: contentCategoryOptions,
  webinars: contentCategoryOptions,
  workshops: contentCategoryOptions,
};
  ...
  try {
    ...
    return {
      courses: Array.isArray(parsed.courses)
        ? uniqueCategoryList(parsed.courses)
        : uniqueCategoryList(defaultContentCategoryMap.courses),
      videos: Array.isArray(parsed.videos)
        ? uniqueCategoryList(parsed.videos)
        : uniqueCategoryList(defaultContentCategoryMap.videos),
      webinars: Array.isArray(parsed.webinars)
        ? uniqueCategoryList(parsed.webinars)
        : uniqueCategoryList(defaultContentCategoryMap.webinars),
      workshops: Array.isArray(parsed.workshops)
        ? uniqueCategoryList(parsed.workshops)
        : uniqueCategoryList(defaultContentCategoryMap.workshops),
    };
  } catch {
    return {
      courses: uniqueCategoryList(defaultContentCategoryMap.courses),
      videos: uniqueCategoryList(defaultContentCategoryMap.videos),
      webinars: uniqueCategoryList(defaultContentCategoryMap.webinars),
      workshops: uniqueCategoryList(defaultContentCategoryMap.workshops),
    };
  }
}

function saveContentCategoryMap(updated: Record<ContentCategoryKey, string[]>) {
  localStorage.setItem("terramatrix_content_categories", JSON.stringify(updated));
}

function updateStoredCategoryReferences(
  group: ContentCategoryKey,
  oldCategory: string,
  newCategory: string
) {
  const oldValue = oldCategory.trim();
  const newValue = newCategory.trim();

  if (!oldValue || !newValue || oldValue === newValue) return;

  const storageKeys: Record<ContentCategoryKey, string[]> = {
    courses: ["terramatrix_courses"],
    videos: ["terramatrix_learning_videos"],
    webinars: ["terramatrix_webinars"],
    workshops: ["terramatrix_workshops"],
  };

  storageKeys[group].forEach((key) => {
    try {
      const saved = localStorage.getItem(key);
      if (!saved) return;

      const updated = (JSON.parse(saved) as { category?: string }[]).map((item) =>
        item.category === oldValue ? { ...item, category: newValue } : item
      );

      localStorage.setItem(key, JSON.stringify(updated));
    } catch {
      // Keep category editing safe even if a localStorage entry is malformed.
    }
  });

  if (group === "webinars" || group === "workshops") {
    try {
      const saved = localStorage.getItem("terramatrix_event_registrations");
      if (!saved) return;

      const expectedKind = group === "webinars" ? "Webinar" : "Workshop";
      const updated = (JSON.parse(saved) as { kind?: string; category?: string }[]).map(
        (item) =>
          item.kind === expectedKind && item.category === oldValue
...
function getManagedCategoryOptions(group: ContentCategoryKey, current = "") {
  const categories = loadContentCategoryMap();
  return uniqueCategoryList([...(categories[group] || []), current]);
}
...
export default function AdminDashboard() {
  ...
  const createOrUpdateCourse = (status: "Draft" | "Published") => {
    ...
    const courseData: Course = {
      ...
      category: courseForm.category || "Industry-Oriented Training",
    ...
    <main>
      ...
      <section style={adminTabs}>
        ...
        <div style={adminLearningDropdownWrap} ref={adminLearningDropdownRef}>
          <button
            type="button"
            onClick={() => setAdminLearningMenuOpen((current) => !current)}
            style={
          ...
          <div style={twoColumn}>
            <Field label="Level">
              <select value={courseForm.level} onChange={(e) => setCourseForm({ ...courseForm, level: e.target.value })} style={inputStyle}>
                {courseLevelOptions.map((level) => <option key={level} value={level}>{level}</option>)}
              </select>
            </Field>
            <Field label="Mode">
      ...
      {activeAdminTab === "categories" && (
        <CategoryAdminPanel
          onCategoryRenamed={(group, oldCategory, newCategory) => {
            if (group === "courses") {
              saveCourses(
                courses.map((course) =>
      ...
      <section style={wideSection}>
        <div style={sectionHeader}>
          <div><div style={eyebrow}>STUDENT ENQUIRIES & ENROLMENT</div><h2 style={panelTitle}>Register Interest Submissions</h2></div>
          <div style={enquiryHeaderActions}>
        ...
        {enquiries.length === 0 ? (
          <div style={emptyEnquiry}>No enquiries yet. When a learner clicks Register Interest, the submission will appear here.</div>
        ) : (
          <div style={enquiryTableWrap}>
            <table style={enquiryTable}>
              <thead>
...
function loadVideoItems() {
  try {
    ...
    if (!saved) return [] as LearningVideo[];
    return (JSON.parse(saved) as Partial<LearningVideo>[]).map(normalizeVideo);
  } catch {
    return [] as LearningVideo[];
  }
}
...
function CategoryAdminPanel({
  ...
  const [categoryMap, setCategoryMap] = useState<Record<ContentCategoryKey, string[]>>(
    loadContentCategoryMap
  );
  const [selectedGroup, setSelectedGroup] = useState<ContentCategoryKey>("videos");
  const [newCategory, setNewCategory] = useState("");
  const [editingCategoryIndex, setEditingCategoryIndex] = useState<number | null>(null);
  const [editingCategoryValue, setEditingCategoryValue] = useState("");

  const updateMap = (updated: Record<ContentCategoryKey, string[]>) => {
    setCategoryMap(updated);
    saveContentCategoryMap(updated);
  };
  ...
  const addCategory = () => {
    ...
    const updated = {
      ...categoryMap,
      [selectedGroup]: uniqueCategoryList([...(categoryMap[selectedGroup] || []), clean]),
    };
  ...
  const startEditCategory = (index: number, value: string) => {
  ...
  const saveEditCategory = (index: number) => {
    ...
    const oldCategory = (categoryMap[selectedGroup] || [])[index] || "";
    ...
    if (clean === oldCategory) {
      resetEditing();
      return;
    }

    const updatedGroup = [...(categoryMap[selectedGroup] || [])];
    updatedGroup[index] = clean;

    updateMap({
      ...categoryMap,
      [selectedGroup]: uniqueCategoryList(updatedGroup),
    });

    updateStoredCategoryReferences(selectedGroup, oldCategory, clean);
    onCategoryRenamed?.(selectedGroup, oldCategory, clean);

    resetEditing();
  };

  const deleteCategory = (index: number) => {
    const updatedGroup = (categoryMap[selectedGroup] || []).filter(
  ...
  return (
    <section style={wideSection}>
      <div style={sectionHeader}>
        <div>
          <div style={eyebrow}>CATEGORY MANAGEMENT</div>
          <h2 style={panelTitle}>Learning Hub Categories</h2>
          <p style={sectionNote}>
            Add, edit or delete category names used in public filters and Admin entry forms.
          </p>
        </div>
        <span style={enquiryCount}>
          {(categoryMap[selectedGroup] || []).length} category(s)
        </span>
      </div>

      <div style={categoryAdminLayout}>
        <div style={categoryGroupList}>
          {(Object.keys(categoryGroupLabels) as ContentCategoryKey[]).map((group) => (
            <button
              key={group}
              type="button"
              onClick={() => {
        ...
        <div id={formAnchorId} style={contentFormCard}>
          <h3 style={miniPanelTitle}>{categoryGroupLabels[selectedGroup]} Categories</h3>
          ...
          <div style={categoryAddRow}>
            ...
            />
            <button type="button" onClick={addCategory} style={primaryButton}>
              Add
            </button>
          </div>

          <div style={categoryRows}>
            {(categoryMap[selectedGroup] || []).map((category, index) => {
              const isEditing = editingCategoryIndex === index;

              return (
                <div key={`${selectedGroup}-${index}-${category}`} style={categoryRow}>
                  <input
                    value={isEditing ? editingCategoryValue : category}
          ...
          <p style={helperText}>
            Use Edit, make changes, then click Save. Public filters update from the saved category list.
          </p>
        </div>
      </div>
    </section>
        ...
        <div style={contentFormCard}>
          ...
          <Field label="Theme / Topic">
            <input
              ...
              style={inputStyle}
              placeholder="Example: QGIS, AutoCAD, Research Writing"
            />
          </Field>
...
function normalizeEventRegistration(item: Partial<EventRegistration>): EventRegistration {
  const kind = item.kind === "Workshop" ? "Workshop" : "Webinar";
  const rawStatus = item.status || "Registered Interest";
...
function EventRegistrationAdminPanel() {
  ...
  const exportRegistrationsCsv = () => {
    ...
    link.href = url;
    link.download = "terramatrix-event-registrations.csv";
    link.click();
    URL.revokeObjectURL(url);
  };
  ...
  return (
    <section style={wideSection}>
      <div style={sectionHeader}>
        <div>
          <div style={eyebrow}>WEBINAR / WORKSHOP REGISTRATIONS</div>
          <h2 style={panelTitle}>Registrations</h2>
          <p style={sectionNote}>
            Webinar and workshop registrations submitted from the public pages
            appear here.
          </p>
        </div>

        <div style={enquiryHeaderActions}>
          <button onClick={exportRegistrationsCsv} style={exportButton}>
            Export CSV
          </button>
          <span style={enquiryCount}>{registrations.length} registration(s)</span>
        </div>
      </div>

      {registrations.length === 0 ? (
        <div style={emptyEnquiry}>
          No webinar/workshop registrations yet.
        </div>
      ) : (
        <div style={enquiryTableWrap}>
          <table style={enquiryTable}>
            ...
            <article key={item.id} style={eventListItem}>
              <div>
                ...
                <p style={contentItemText}>
                  {item.date || "Date TBA"} {item.time || ""} · {item.mode} ·{" "}
                  Instructor(s): {item.resourcePerson || "TerraMatrix Faculty"} · Participation Certificate
...
function Detail({ label, value }: { label: string; value: string }) {
  return <div style={detailBox}><span>{label}</span><strong>{value || "Not provided"}</strong></div>;
}
...
const messageModal: CSSProperties = { background: "#FFFFFF", borderRadius: "22px", maxWidth: "760px", width: "100%", maxHeight: "85vh", overflow: "auto", position: "relative", padding: "32px", textAlign: "left", boxShadow: "0 24px 70px rgba(0,0,0,0.22)" };
const modalCloseButton: CSSProperties = { position: "absolute", top: "14px", right: "16px", background: "#FFFFFF", border: "1px solid #E8E1D2", borderRadius: "50%", width: "36px", height: "36px", fontSize: "24px", cursor: "pointer", color: "#173F35" };
const modalTitle: CSSProperties = { color: "#173F35", fontSize: "28px", margin: "0 0 6px" };
const modalSubTitle: CSSProperties = { color: "#53665E", fontSize: "17px", fontWeight: 800, margin: "0 0 20px" };
const modalInfoGrid: CSSProperties = { display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: "12px", marginBottom: "16px" };
const detailBox: CSSProperties = { background: "#FBFAF6", border: "1px solid #E8E1D2", borderRadius: "12px", padding: "12px", display: "grid", gap: "5px" };
const messageBox: CSSProperties = { background: "#FBFAF6", border: "1px solid #E8E1D2", borderRadius: "12px", padding: "14px", marginBottom: "14px" };
